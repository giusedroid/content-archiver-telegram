...
Search index files generated by `/search` are treated as runtime artifacts. The container
removes or restores `index/lancedb-manifest.yml`, `index/index-report.json`, and
`index/semantic-records.jsonl` before startup sync when `CLEAN_GENERATED_INDEX_ON_STARTUP=true`.
This keeps generated search state from blocking future container restarts.
